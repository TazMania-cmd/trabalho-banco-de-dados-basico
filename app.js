...
try {
  ...
  app.use('/api/auth', ambienteValido, criarAuthRoutes({ supabase, garantirAdminInicial }));
} catch (error) {
  console.error('Falha ao configurar autenticação:', error);
  errosAmbiente.push('Falha ao configurar autenticação.');
}
...
app.get('/', (req, res) => {
  if (req.session?.usuario) return res.sendFile(path.join(publicDir, 'index.html'));
